        ...
        {tripPlans.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mt-6">
            ...
            {tripPlans.map((trip) => (
              <TripCard
                key={trip.id}
                trip={formatTripForCard(trip)} 
                onTripDeleted={(id)=>{setTripPlans([...tripPlans.filter((trip) => trip.id !== id)])}}
                />
            ))}
          </div>
        ) : (
...
function TripCard({ trip,onTripDeleted }) {
  const categoryLabels = { Hotel: "🏨", Restaurant: "🍽️", "Popular Place": "📍" };
  const breakdown = Object.entries(trip.categoryCounts || {});
  const [isDeleting, setIsDeleting] = useState(false);
  const { deletePlan } = useTripPlanStore();

  const handleDeletePlan = useCallback(async (e, id) => {
    console.log(id)
    e.preventDefault();
    e.stopPropagation();
      ...
      <div className="bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-xl hover:scale-[1.02] transition duration-300 border border-gray-100 flex flex-col text-left">
        <div className="relative overflow-hidden h-48">
          ...
          <button
            ...
            className="absolute top-3 right-3 bg-white/90 backdrop-blur-md text-gray-600 hover:text-red-600 hover:bg-white p-2 rounded-full shadow-sm transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isDeleting ? (
