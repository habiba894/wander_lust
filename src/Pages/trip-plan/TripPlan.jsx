...
import TripPlanActionBar from "../../Components/trip-plan/TripPlanActionBar";
import TripPlanBanner from "../../Components/trip-plan/TripPlanBanner";
import TripPlanCountryMap from "../../Components/trip-plan/TripPlanCountryMap";
import TripPlanEmptyState from "../../Components/trip-plan/TripPlanEmptyState";
import TripPlanNotes from "../../Components/trip-plan/TripPlanNotes";
import TripPlanPlacesSidebar from "../../Components/trip-plan/TripPlanPlacesSidebar";
import TripPlanSuggestion from "../../Components/trip-plan/TripPlanSuggestion";
import TripPlanUserSelections from "../../Components/trip-plan/TripPlanUserSelections";
import { apiServices } from "../../services/api";
import useCountryStore from "../../stores/countryStore";
import useTripPlanStore, {
  isPlanLocked
} from "../../stores/planStore";

const randomInt = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;

const pickRandomOne = (list) => {
  if (!list || list.length === 0) return [];
  return [list[randomInt(0, list.length - 1)]];
};

const pickRandomMany = (list) => {
  if (!list || list.length === 0) return [];
  const count = randomInt(1, list.length);
...
export default function TripPlanPage() {
  ...
  const isSuggested = Boolean(location.state?.isSuggested);
  const { countryName: countryFromUrl, planId } = useParams();


  const { setCountryDetails } = useCountryStore();
  const {
    plan: tripPlan,
    isSaving,
    isLoading,
  ...
  const isEditMode = Boolean(planId);
  const resolvedCountryName = countryFromUrl ?? tripPlan.destination;
  const locked = isEditMode && isPlanLocked(tripPlan.startDate);



  const [suggestions, setSuggestions] = useState({
    hotels: [], restaurants: [], PopularPlaces: [],
  });


  // ------------------------------------------------------
  // LOAD PLAN (edit mode) OR INITIALIZE DRAFT (create/suggestion mode)
  // ------------------------------------------------------
  useEffect(() => {
    if (isEditMode) {
      loadPlanById(planId);
  ...
  // ------------------------------------------------------
  useEffect(() => {
    console.log(resolvedCountryName)
    if (!resolvedCountryName) return;
    ...
    const fetchCountryDetails = async () => {
      try {
        const res = await apiServices.getCountryDetails(resolvedCountryName);
        setSuggestedPlaces({
          hotels: pickRandomOne(res.hotels) || [],
          restaurants: pickRandomMany(res.restaurants) || [],
          PopularPlaces: pickRandomMany(res.places) || [],
        });
        setCountryDetails(res);
        console.log(res)
      } catch (err) {
  ...
  // ------------------------------------------------------
  // SUGGESTION MODE RENDER — separate, simpler screen
  // ------------------------------------------------------
  if (isSuggested) {
    return (
      <div className="min-h-screen bg-[#f7f4fa]">
        <main className="max-w-5xl mx-auto px-6 py-12">
          <TripPlanBanner
            ...
            planName={`A trip to ${resolvedCountryName || "your destination"}, picked for you`}
          />

          <div className="mt-10">
            <h2 className="text-3xl font-bold text-gray-800 mb-2">Your Suggested Trip</h2>
            <p className="text-gray-500 mb-8">
              We picked a starter itinerary for
              <span className="font-semibold"> {resolvedCountryName[0].toUpperCase() + resolvedCountryName.slice(1)}</span>.
              Confirm to save it, or discard to choose your own.
            </p>

            <TripPlanSuggestion countryName={resolvedCountryName} />
  ...
  // ------------------------------------------------------
  // STANDARD RENDER — create or edit mode
  // ------------------------------------------------------
  return (
    <div className="min-h-screen bg-[#f7f4fa]">
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {locked && (
          <div className="mt-6 bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl px-5 py-4 text-sm font-medium">
            🔒 This trip has already started, so it can no longer be edited.
          </div>
        )}

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-8 items-stretch">
          {!locked && (
            <TripPlanPlacesSidebar
              places={suggestions}
              isLoading={false}
            />
          )}

          <TripPlanCountryMap location={resolvedCountryName} />

          <TripPlanNotes locked={locked} />
        </div>

        <div className="mt-12">
          {isTripEmpty ? (
            <TripPlanEmptyState countryName={resolvedCountryName} />
          ) : (
            <div className="space-y-12">
              <h3 className="text-3xl font-bold text-gray-800 mb-8 border-b pb-4">Your Plan</h3>
              {Object.entries(placesByCategory).map(([category, places]) => (
                <TripPlanUserSelections
                  key={category}
                  places={places}
                  locked={locked}
                  category={category}
        ...
        {!locked && tripPlan.places.some((p) => p.tag !== "load") && (<TripPlanActionBar />)}
      </main>
    </div>
  );
}
